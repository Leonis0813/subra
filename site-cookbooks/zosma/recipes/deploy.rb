...
deploy node[:zosma][:deploy_dir] do
  ...
  before_migrate do
    ...
    node[:zosma][:mysql_users].each do |user|
  ...
  before_restart do
    ...
    gmail = Chef::EncryptedDataBagItem.load('zosma', 'gmail')
    ...
    execute 'rvm 2.2.0 do bundle exec rake assets:precompile' do
      cwd release_path
      environment 'RAILS_ENV' => 'production', 'PATH' => node[:rvm][:path], 'RAILS_RELATIVE_URL_ROOT' => "/#{node[:zosma][:app_name]}"
      only_if { node.chef_environment == 'compute' }
    end
  ...
  restart_command do
    execute 'rvm 2.2.0 do bundle exec rake unicorn:stop' do
      ...
      only_if "pgrep -lf 'unicorn_rails.*#{node[:zosma][:app_name]}*'"
    end
